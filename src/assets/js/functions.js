...
var shouldrungalleryanim = false;
...
function updateSaleProgress(supply) {
  ...
  var salephases = [4000,7000,10000,13000,13333,13336,13337];
  ...
  for(var i = 0; i < salephases.length; i++) {
    ...
    console.log(i, "adding relative supply", salephases[i]);
  }
...
function randomGalleryImg() {
  if(shouldrungalleryanim) {
    var newrandomimg = getRandomInt(1,13337);
    var position = getRandomInt(0,7);

    var imgurl = "rocketgallery/img/thumb/lil-moon-rocket-"+newrandomimg+"-thumb.png";

    // Create a temporary image.
    var img = new Image();

    $(img).on('load',function() {
      $('#hpgallery img:eq('+position+')').fadeOut('slow', function() {
...
function enableGallerySwap() {
  shouldrungalleryanim = true;
}

function disableGallerySwap() {
  shouldrungalleryanim = false;
}

$(document).ready(function() {
  setInterval(randomGalleryImg, 1000);

  $('#hpgallery').on('touchstart', enableGallerySwap);
  $('#hpgallery').on('touchmove', enableGallerySwap);
  $('#hpgallery').on('mouseenter', enableGallerySwap);

  $('#hpgallery').on('touchend', disableGallerySwap);
  $('#hpgallery').on('mouseleave', disableGallerySwap);
});
